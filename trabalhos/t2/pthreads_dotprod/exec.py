...
#tam_vet = 1000000
#tam_vet = 500 000
tam_vet = float(sys.argv[3])
for repeat in range(500, 3000, 500):
	for pot in range(0, 4):
		nthreads = pow(2,pot)

		os.system('echo "' + '" >> ' + sys.argv[2])
		os.system('echo "' + sys.argv[1] + ' ' + str(nthreads) + ' ' + str(tam_vet/nthreads) + ' ' + str(repeat) + '" >> ' + sys.argv[2])		
		for i in range(0, 20):
			print(sys.argv[1] + ' ' + str(nthreads) + ' ' + str(tam_vet/nthreads) + ' ' + str(repeat) + " >> " + sys.argv[2])
			os.system(sys.argv[1] + ' ' + str(nthreads) + ' ' + str(tam_vet/nthreads) + ' ' + str(repeat) + " >> " + sys.argv[2])
